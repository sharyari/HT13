...
avdc_access(avdark_cache_t *self, avdc_pa_t pa, avdc_access_type_t type)
{
		...
		for (int i = 0; i < self->assoc; i++){
			...
			if(hit) {
				self->sets[index].lines[i].age = -1;
				found = 1;
				break;
			} else if(self->sets[index].lines[i].valid && self->sets[index].lines[i].age > highestAge) {
				highestAgeIndex = i;
				highestAge = self->sets[index].lines[i].age;
			} else if(!self->sets[index].lines[i].valid) {
				highestAgeIndex = i;
				highestAge = 50000;
			} else {
				;
		...
		for(int i = 0; i < self->assoc; i++)
			self->sets[index].lines[i].age++;

		if(!found) {
			self->sets[index].lines[highestAgeIndex].valid = 1;
    	    self->sets[index].lines[highestAgeIndex].tag = tag;
    	    self->sets[index].lines[highestAgeIndex].age = 0;
		}

        switch (type) {
        case AVDC_READ: /* Read accesses */
                avdc_dbg_log(self, "read: pa: 0x%.16lx, tag: 0x%.16lx, index: %d, hit: %d\n",
                             (unsigned long)pa, (unsigned long)tag, index, found);
...
avdc_flush_cache(avdark_cache_t *self)
{
   ...
   for (int i = 0; i < self->number_of_sets; i++) {
		for (int j = 0; j<self->assoc; j++){
                self->sets[i].lines[j].valid = 0;
				self->sets[i].lines[j].age = 0;
                self->sets[i].lines[j].tag = 0;
		}
   }
}
        ...
        for (i = 0; i < self->number_of_sets; i++)
			for (j = 0; j < self->assoc; j++)
                fprintf(stderr, "tag: <0x%.16lx> valid: %d\n",
                        (long unsigned int)self->sets[i].lines[j].tag,
                        self->sets[i].lines[j].valid);
}
